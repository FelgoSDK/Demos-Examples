...
Item {
  ...
  FirebaseAuth {
    ...
    onAuthenticatedChanged: if(auth.authenticated) dataModel.loggedIn()
  }
  ...
  FirebaseDatabase {
    ...
    onRealtimeUserValueChanged: (successs, key, value) => { if(key === dbKeyAllShoppingItems) shoppingItemsLoaded(value) }
    ...
    onWriteCompleted: (success, message) => console.log("write completed")
  }
